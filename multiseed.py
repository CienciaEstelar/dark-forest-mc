"""
Multi-seed robustness analysis for Dark Forest Monte Carlo.

Runs paso5 with 10 seeds and reports variance across runs:
  - Destructions (n and %)
  - Mean nearest-neighbor distance
  - Observed/Poisson ratio

Run from ~/Escritorio/:
    python -m bosque_oscuro.multiseed
"""
...
OUTPUT_PATH = Path(__file__).parent / "output_sim" / "multiseed_results.json"


def _simular_y_analizar(seed: int, p: Parametros) -> dict:
    np.random.seed(seed)
    ...
    tmp_dir = OUTPUT_PATH.parent / "multiseed_tmp"
...
def main() -> None:
    ...
    print(f"Multi-seed analysis — {len(SEEDS)} seeds, escenario pesimista")
    print(f"Seeds: {SEEDS}\n")

    resultados = []
    for seed in SEEDS:
        print(f"  Seed {seed:>5} ...", end=" ", flush=True)
        r = _simular_y_analizar(seed, p)
        resultados.append(r)
        print(
            f"destruidas={r['destruidas']:>3} ({r['pct_destruidas']:>5.2f}%)  "
